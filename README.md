...
- 🚦 **Pace-Ampel je Fenster**: `green`/`yellow`/`red` je nach Prognose, Grenzen
  einstellbar über **Warning Threshold** (Grün/Gelb) und **Pace Red Limit** (Gelb/Rot,
...
- 📦 **Modell-Katalog als EINEN Sensor** (`sensor.go_gauge_models`): komplettes,
...
- 🏆 Kompakt-Sensoren: Live Models · Cheapest Model · Free Models
- 🔘 **Refresh-Button** „Refresh" — holt sofort beides (Nutzung + Modelle),
...
3. Optionen (Zahnrad auf dem Gerät) **oder** die Switch-/Number-Entities des Geräts:
   Refresh-Zyklen und Warning Threshold nach Geschmack einstellen
...
Reset-Zeiten, Refresh-/Warning-Threshold-Settings). Die workspace-unabhängigen Entities
(Model Catalog, Live Models, Cheapest Model, Free Models, API Reachable) liegen
...
| `sensor.go_gauge_team_monthly_usage` | 0 (%) — bei fehlendem Abo: `Kein Abo` |
| `sensor.go_gauge_team_monthly_reset` | `2026-09-17T08:38:28+00:00` (Timestamp) |
| `sensor.go_gauge_team_weekly_forecast` | z. B. 92.4 (%) — Hochrechnung aufs Fensterende, kann >100 sein |
...
| `sensor.go_gauge_cheapest_model` | z. B. `muse-spark-1.2-contributor` (Gerät „Go Gauge Konto") |
| `sensor.go_gauge_free_models` | `ox-alpha-free` (Gerät „Go Gauge Konto") |
| `button.go_gauge_refresh` | sofortiger Gesamt-Refresh |
...
| `number.go_gauge_models_refresh_min_team` | Models-Refresh-Intervall in Minuten (nur Catalog Owner) |
| `switch.go_gauge_team_auto_update_usage` | Auto-Refresh Nutzung ein/aus |
| `switch.go_gauge_team_auto_update_models` | Auto-Refresh Modell-Katalog ein/aus (nur Catalog Owner) |
